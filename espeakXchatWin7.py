...
def espeaking1(chan, lang, nick):
		...
		if (lang == 'brazil'):
			...
			audioI+=1
			audioS=audioI
		subprocess.Popen('C:\\Program Files\\eSpeak\\command_line\\espeak.exe -g 10 --pho -p 30 -s 170 -v %s -f '
                                 'F:\\temp\\teste-%s-%s ' % (lang, chan[1:], nick), shell=False)
		subprocess.Popen('C:\\Program Files\\eSpeak\\command_line\\espeak.exe -w F:\\temp\\%03d.%s-%s.wav -g 10 '
                                 '--pho -p 30 -s 170 -v %s -f F:\\temp\\teste-%s-%s '
                                 % (audioS, lang, nick, lang, chan[1:], nick), shell=False)
...
def falar_(word, word_eol, userdata):
	...
	for lang,canal in enumerate(channels):
			if (current == canal):
				with open("F:/temp/teste-%s-%s" % (current[1:], nickname), 'w') as f:
					f.write(palavras)
